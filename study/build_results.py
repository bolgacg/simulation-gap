...
def main():
    ...
    out = {
        ...
        "statistics_only_finding": {
            ...
            "motion_axis_DOES_NOT_clear_the_floor": "drag anisotropy is registered and "
                "cannot be resolved. At ten clips per configuration the wider setting was "
                "closer to real by 0.013 and the tighter one changed side; at fifty the signs "
                "are consistent but the estimate has halved to 0.006 with a scatter of 0.003, "
                "a ratio of 2.0 that is still under the bar of three this page uses. An "
                "estimate that halves when the measurement improves fivefold is behaving like "
                "an effect near zero rather than like a small real one, so the honest reading "
                "is that these statistics register alpha at about a fiftieth of what the body "
                "radius does and cannot resolve it. That makes the physics point harder rather "
                "than softer: alpha is the one parameter that can be shown wrong from first "
                "principles, drawn with a median near 5 where slender-body theory says 1.5 to "
                "2 and only 4.8 percent of draws in that range, and moving it to the physical "
                "value changes these statistics by nothing they can measure. A method that "
                "picks simulator settings by matching unlabelled statistics would not find the "
